...
#define SERIAL_FLASHER_PORT Serial1
...
SERIAL_FLASHER_PORT.begin(115200);
...
{
   size_t err = SERIAL_FLASHER_PORT.write((const char *)data, size);
...
{
	SERIAL_FLASHER_PORT.setTimeout(timeout);
    int read = SERIAL_FLASHER_PORT.readBytes( data, size);
...
{
    SERIAL_FLASHER_PORT.begin(baudrate);
    int err = SERIAL_FLASHER_PORT;
 ...
 esp_loader_connect_args_t connect_config = ESP_LOADER_CONNECT_DEFAULT();
    ...
    if (higher_baudrate && esp_loader_get_target() != ESP8266_CHIP) {
        ...
        if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
            ...
            err = loader_port_change_transmission_rate(higher_baudrate);
            if (err != ESP_LOADER_SUCCESS) {
